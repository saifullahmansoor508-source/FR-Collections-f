...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_slide':
                $title = sanitizeInput($_POST['header_heading']);
                $description = sanitizeInput($_POST['header_subheading']);
                $button_text = sanitizeInput($_POST['header_button']);
                ...
                if ($image_path) {
                    $stmt = $db->prepare("INSERT INTO slider_images (image_path, title, description, button_text, button_link, sort_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    if ($stmt->execute([$image_path, $title, $description, $button_text, $button_link, $sort_order, $is_active])) {
                        $success_message = "Slider image added successfully!";
                    } else {
                        $error_message = "Failed to add slider image.";
                    }
                } else {
            ...
            case 'update_slide':
                ...
                $stmt = $db->prepare("UPDATE slider_images SET image_path = ?, title = ?, description = ?, button_text = ?, button_link = ?, sort_order = ?, is_active = ? WHERE id = ?");
                if ($stmt->execute([$image_path, $title, $description, $button_text, $button_link, $sort_order, $is_active, $id])) {
                    $success_message = "Slider image updated successfully!";
                } else {
                    $error_message = "Failed to update slider image.";
                }
                break;
            ...
            case 'delete_slide':
                ...
                if ($slide) {
                    ...
                    if ($stmt->execute([$id])) {
                        ...
                        $error_message = "Failed to delete slider image.";
                    }
                }
                break;
                
            case 'bulk_delete_slides':
                if (isset($_POST['slide_ids']) && is_array($_POST['slide_ids'])) {
                    ...
                    if ($deleted_count > 0) {
                        $success_message = "$deleted_count slide(s) deleted successfully!";
                    } else {
                        $error_message = "Failed to delete slides.";
                    }
                }
                break;
        }
...
$stmt = $db->prepare("SELECT COUNT(*) FROM slider_images");
$stmt->execute();
$total_slides = $stmt->fetchColumn();

$stmt = $db->prepare("SELECT COUNT(*) FROM slider_images WHERE is_active = 1");
$stmt->execute();
$active_slides = $stmt->fetchColumn();

$stmt = $db->prepare("SELECT COUNT(*) FROM slider_images WHERE is_active = 0");
$stmt->execute();
$inactive_slides = $stmt->fetchColumn();

$stmt = $db->prepare("SELECT MAX(sort_order) FROM slider_images");
$stmt->execute();
        ...
        <div class="page-header-card">
            <div class="page-header-content">
                ...
                <div class="page-header-text">
                    <h1 class="page-title">Homepage Management</h1>
                    <p class="page-subtitle">Manage homepage slider and content</p>
                </div>
            </div>
            <div class="stats-grid">
                <div class="stats-card-simple">
                    <div class="stats-content-simple">
                        <h3><?php echo number_format($total_slides); ?></h3>
                        <p>Total Slides</p>
                    </div>
                </div>
                ...
                <div class="stats-card-simple">
                    <div class="stats-content-simple">
                        <h3><?php echo number_format($active_slides); ?></h3>
                        <p>Active Slides</p>
                    </div>
                </div>
                ...
                <div class="stats-card-simple">
                    <div class="stats-content-simple">
                        <h3><?php echo number_format($inactive_slides); ?></h3>
                        <p>Inactive Slides</p>
                    </div>
                </div>
        ...
        <div class="reviews-container">
            <div class="reviews-header">
                <div class="reviews-header-content">
                    ...
                    <div class="reviews-title">
                        <h4>Slider Images</h4>
                        <span class="reviews-count"><?php echo count($slider_images); ?> slides</span>
                    </div>
                </div>
                <div class="reviews-actions-icons">
                    <button class="header-icon-btn select-icon" onclick="toggleSelectAllSlides()" id="selectAllBtnSlides" title="Select All">
                        <i class="fas fa-check-double"></i>
                    </button>
                    <button class="header-icon-btn delete-icon" onclick="bulkDeleteSlides()" id="deleteSelectedBtnSlides" title="Delete Selected">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                    <button class="header-icon-btn refresh-icon" onclick="location.reload()" title="Refresh List">
                        <i class="fas fa-sync-alt"></i>
                    </button>
                    <button class="header-icon-btn add-icon" data-bs-toggle="modal" data-bs-target="#addSlideModal" title="Add New Slide">
            ...
            <?php if (empty($slider_images)): ?>
                <div class="empty-reviews">
                    <div class="empty-reviews-icon"><i class="fas fa-images"></i></div>
                    <h5>No slider images found</h5>
                    <p>Add your first slider image to get started.</p>
                    <button class="btn btn-primary-modern" data-bs-toggle="modal" data-bs-target="#addSlideModal">
            ...
            <?php else: ?>
                <div class="reviews-grid">
                    <?php foreach ($slider_images as $slide): ?>
                        <div class="review-card <?php echo !$slide['is_active'] ? 'pending-review' : ''; ?>">
                            <div class="review-card-header">
                                <div class="desktop-checkbox-section">
                                    <input type="checkbox" class="slide-checkbox desktop-checkbox" value="<?php echo $slide['id']; ?>">
                                </div>
                                <div class="review-user-info">
                                    ...
                                    <div class="user-details-review">
                                        <div class="user-name-review">Slide #<?php echo $slide['id']; ?></div>
                            ...
                            <div class="review-card-body">
                                ...
                                <div class="product-info-review">
                                    ...
                                    <span class="product-name-review"><?php echo htmlspecialchars($slide['title']); ?></span>
                                </div>

                                <div class="rating-display">
                                    <i class="fas fa-text-height me-2" style="color: #6b7280;"></i>
                                    <span class="rating-text">
                                        <?php echo htmlspecialchars(substr($slide['description'], 0, 30)) . (strlen($slide['description']) > 30 ? '...' : ''); ?>
                                    </span>
                                </div>
                                ...
                                <div class="slide-details">
                                    <div class="detail-row">
                                        <div class="detail-item">
                                            <i class="fas fa-button me-2" style="color: #10b981;"></i>
                                            <strong>Button:</strong>
                                            <span class="button-text"><?php echo $slide['button_text'] ?: 'No button'; ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <i class="fas fa-link me-2" style="color: #3b82f6;"></i>
                                            <strong>Link:</strong>
                                            <span class="link-text"><?php echo $slide['button_link'] ?: 'No link'; ?></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="review-card-footer">
                                <div class="review-actions-icons">
                                    <button type="button" class="action-icon-btn edit-icon" title="Edit Slide"
                                            onclick="editSlide(<?php echo htmlspecialchars(json_encode($slide)); ?>)">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    
                                    <form method="POST" class="d-inline" onsubmit="return handleDeleteConfirm(event, 'This slide will be permanently deleted. Are you sure?', 'Delete Slide')">
                                        <input type="hidden" name="action" value="delete_slide">
                                        <input type="hidden" name="slide_id" value="<?php echo $slide['id']; ?>">
                                        <button type="submit" class="action-icon-btn delete-icon" title="Delete Slide">
                                            <i class="fas fa-trash-alt"></i>
                                        </button>
                                    </form>
                                </div>
                ...
                <div class="mobile-action-icons">
                    <button class="mobile-icon-btn select-icon" onclick="toggleSelectAllSlides()" id="selectAllBtnSlidesMobile" title="Select All">
                        <i class="fas fa-check-double"></i>
                    </button>
                    <button class="mobile-icon-btn delete-icon" onclick="bulkDeleteSlides()" id="deleteSelectedBtnSlidesMobile" title="Delete Selected">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                    <button class="mobile-icon-btn refresh-icon" onclick="location.reload()" title="Refresh">
                        <i class="fas fa-sync-alt"></i>
                    </button>
                    <button class="mobile-icon-btn add-icon" data-bs-toggle="modal" data-bs-target="#addSlideModal" title="Add Slide">
                        <i class="fas fa-plus"></i>
                    </button>
                </div>
                    ...
                    <?php foreach ($slider_images as $slide): ?>
                        <div class="user-list-item">
                            <div class="user-list-header" onclick="toggleDetails(this)">
                                <div class="slide-checkbox-section">
                                    <input type="checkbox" class="slide-checkbox" value="<?php echo $slide['id']; ?>" onclick="event.stopPropagation()">
                                </div>
                                <div class="user-list-avatar" style="background: linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%);">
                                    <i class="fas fa-image"></i>
                                </div>
                                <div class="user-info-primary">
                                    <div class="user-name-mobile">Slide #<?php echo $slide['id']; ?></div>
                            ...
                            <div class="user-list-details collapsed">
                                <div class="detail-row-mobile">
                                    ...
                                    <span class="detail-value"><?php echo htmlspecialchars($slide['title']); ?></span>
                                </div>
                                <div class="detail-row-mobile">
                                    <i class="fas fa-text-height detail-icon"></i>
                                    <span class="detail-label">Description:</span>
                                    <span class="detail-value"><?php echo htmlspecialchars(substr($slide['description'], 0, 40)) . '...'; ?></span>
                                </div>
                                <div class="detail-row-mobile">
                                    <i class="fas fa-sort detail-icon"></i>
                                    <span class="detail-label">Order:</span>
                                    <span class="detail-value"><?php echo $slide['sort_order']; ?></span>
                                </div>
                                <div class="detail-row-mobile">
                                    <i class="fas fa-link detail-icon"></i>
                                    <span class="detail-label">Button:</span>
                                    <span class="detail-value"><?php echo $slide['button_text'] ?: 'No button'; ?></span>
                                </div>
                                
                                <div class="user-actions-mobile">
                                    <button class="mobile-btn mobile-btn-view" onclick="editSlide(<?php echo htmlspecialchars(json_encode($slide)); ?>)">
                                        <i class="fas fa-edit"></i> Edit
                                    </button>
                                    <form method="POST" class="d-inline" onsubmit="return handleDeleteConfirm(event, 'Delete this slide?', 'Delete')">
                                        <input type="hidden" name="action" value="delete_slide">
                                        <input type="hidden" name="slide_id" value="<?php echo $slide['id']; ?>">
        ...
        <div class="modal-content" style="border-radius: 20px; border: none; overflow: hidden;">
            <div class="modal-header" style="background: linear-gradient(135deg, #0058a3 0%, #ff6b00 100%); color: white; border: none; padding: 25px 30px;">
                <h5 class="modal-title" style="font-weight: 700; font-size: 1.5rem; display: flex; align-items: center; gap: 10px;">
                    <i class="fas fa-plus-circle"></i><span>Add New Slider Image</span>
                </h5>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="modal-body" style="padding: 30px;">
                    ...
                    <div class="form-group-modern">
                        ...
                        <div class="form-text-modern">Recommended size: 1920x600px</div>
                    </div>
                    
                    <div class="form-group-modern">
                        <label for="header_heading" class="form-label-modern">
                            <i class="fas fa-heading me-2"></i>Header Heading *
                        </label>
                        <input type="text" class="form-control-modern" id="header_heading" name="header_heading" required>
                    </div>
                    
                    <div class="form-group-modern">
                        <label for="header_subheading" class="form-label-modern">
                            <i class="fas fa-text-height me-2"></i>Header Subheading
                        </label>
                        <textarea class="form-control-modern" id="header_subheading" name="header_subheading" rows="2"></textarea>
                    </div>
                    
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                <label for="header_button" class="form-label-modern">
                                    <i class="fas fa-button me-2"></i>Button Text
                                </label>
                                <input type="text" class="form-control-modern" id="header_button" name="header_button" placeholder="e.g. Shop Now">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                <label for="button_link" class="form-label-modern">
                                    <i class="fas fa-link me-2"></i>Button Link
                                </label>
                                <input type="text" class="form-control-modern" id="button_link" name="button_link" placeholder="e.g. shop.php">
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="editSlideModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="border-radius: 20px; border: none; overflow: hidden;">
            <div class="modal-header" style="background: linear-gradient(135deg, #0058a3 0%, #ff6b00 100%); color: white; border: none; padding: 25px 30px;">
                <h5 class="modal-title" style="font-weight: 700; font-size: 1.5rem; display: flex; align-items: center; gap: 10px;">
                    <i class="fas fa-edit"></i><span>Edit Slider Image</span>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
                ...
                <div class="modal-body" style="padding: 30px;">
                    ...
                    <div class="form-group-modern">
                        ...
                        <div class="current-image-preview">
                            <img id="edit_current_image_preview" src="" alt="Current Image" 
                                 class="current-image">
                        </div>
                    </div>
                    
                    <div class="form-group-modern">
                        <label for="edit_image" class="form-label-modern">
                            <i class="fas fa-sync me-2"></i>New Image (optional)
                        </label>
                        <input type="file" class="form-control-modern" id="edit_image" name="image" accept="image/*">
                        <div class="form-text-modern">Leave empty to keep current image</div>
                    </div>
                    
                    <div class="form-group-modern">
                        <label for="edit_header_heading" class="form-label-modern">
                            <i class="fas fa-heading me-2"></i>Header Heading *
                        </label>
                        <input type="text" class="form-control-modern" id="edit_header_heading" name="header_heading" required>
                    </div>
                    
                    <div class="form-group-modern">
                        <label for="edit_header_subheading" class="form-label-modern">
                            <i class="fas fa-text-height me-2"></i>Header Subheading
                        </label>
                        <textarea class="form-control-modern" id="edit_header_subheading" name="header_subheading" rows="2"></textarea>
                    </div>
                    
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                <label for="edit_header_button" class="form-label-modern">
                                    <i class="fas fa-button me-2"></i>Button Text
                                </label>
                                <input type="text" class="form-control-modern" id="edit_header_button" name="header_button">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                <label for="edit_button_link" class="form-label-modern">
                                    <i class="fas fa-link me-2"></i>Button Link
                                </label>
                                <input type="text" class="form-control-modern" id="edit_button_link" name="button_link">
                            </div>
                        </div>
                    </div>
...
function editSlide(slide) {
    ...
    document.getElementById('edit_current_image_preview').src = '../' + slide.image_path;
    document.getElementById('edit_header_heading').value = slide.title;
    document.getElementById('edit_header_subheading').value = slide.description;
    document.getElementById('edit_header_button').value = slide.button_text || '';
...
<style>
/* ===== COPY ALL BASE STYLES FROM PREVIOUS PAGES ===== */
...
@media (max-width: 768px) {
    ...
    .slide-checkbox-section { display: flex !important; }
...
<script>
// Select All / Deselect All for slides
let allSlidesSelected = false;
...
function toggleSelectAllSlides() {
    ...
    checkboxes.forEach(cb => cb.checked = allSlidesSelected);
    
    // Update desktop button
    const btnDesktop = document.getElementById('selectAllBtnSlides');
    if (btnDesktop) {
    ...
    const btnMobile = document.getElementById('selectAllBtnSlidesMobile');
    if (btnMobile) {
        if (allSlidesSelected) {
...
async function bulkDeleteSlides() {
    ...
    if (uniqueIds.length === 0) {
        showAlert('Please select at least one slide to delete', 'warning', 'No Selection');
        return;
    }
    
    if (await showConfirm(`Delete ${uniqueIds.length} selected slide(s)?`, 'Bulk Delete', {confirmText: 'Yes, Delete', cancelText: 'Cancel', type: 'danger'})) {
        ...
        actionInput.value = 'bulk_delete_slides';
